    ...
    def onclick(self, event):
        ...
        ##            myWorkbook.add_format({'align':'center','valign':'vcenter'})
                    ...
                    if self.checkDrawThresh.get():
                        plt.plot((Time[0], Time[-1]), (avgThrsh, avgThrsh), 'r--')
                    ...
                    if self.checkDrawLine.get():
                        for k in range(0,nPeaks):
                            ...
                            plt.plot( ( Time[int(myMinIndexes[k])], Time[int(myTrigIndex[k])] ) , ( tempVUno, tempVDos ), 'b:')
...
if __name__ == "__main__":
    ...
    app.title('APAnalyzer v5 by Jorge Castorena')
    app.mainloop()
